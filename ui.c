...
static const struct { gr_surface* surface; const char *name; } BITMAPS[] = {
    ...
    { &gBackgroundIcon[BACKGROUND_ICON_CLOCKWORK],  "icon_skull" },
...
void ui_init_icons(void) {
    ...
    for (i = 0; BITMAPS[i].name != NULL; ++i) {
        int result = res_create_recovery_surface(BITMAPS[i].name, BITMAPS[i].surface);
        if (result < 0) {
...
void ui_init(void)
{
    ...
    for (i = 0; i < ui_parameters.indeterminate_frames; ++i) {
        ...
        int result = res_create_recovery_surface(filename, gProgressBarIndeterminate+i);
        if (result < 0) {
            LOGE("Missing bitmap %s\n(Code %d)\n", filename, result);
        }
    }
    ...
    if (ui_parameters.installing_frames > 0) {
        ...
        for (i = 0; i < ui_parameters.installing_frames; ++i) {
            ...
            int result = res_create_recovery_surface(filename, gInstallationOverlay+i);
            if (result < 0) {
                LOGE("Missing bitmap %s\n(Code %d)\n", filename, result);
            }
        }
